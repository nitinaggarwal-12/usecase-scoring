...
clean_premium_code = """import { useState, useMemo, useEffect, useRef, useCallback } from 'react';
import { 
...
export default function PremiumScopingAssessorV11({ onBackToLanding, globalTheme = 'dark', apiKey = '', apiKey2 = '', gcpToken = '' }) {
  ...
  // --------------------------------------------------------------------------
  // Dynamic Premium Theme Styling Tokens (Boardroom Slate System)
  // --------------------------------------------------------------------------
  const t = {
    bg: 'radial-gradient(circle at top right, #0a0f1d, #05070f)',
    cardBg: 'rgba(18, 25, 44, 0.65)',
    ...
    btnBg: 'rgba(56, 189, 248, 0.06)',
    btnHover: 'rgba(56, 189, 248, 0.18)',
    pillCurrent: 'linear-gradient(135deg, #0284c7, #0ea5e9)', // Deep Sapphire Cyan
    pillTarget: 'linear-gradient(135deg, #059669, #10b981)', // Deep Emerald Green
    accentGreen: '#10b981'
  };
  ...
  const scoringData = useMemo(() => {
    ...
    if (finalPriorityScore > 0) {
      if (finalPriorityScore >= 85) {
        decision = 'Launch Now';
        activationImpact = '$1.4M Annual Value';
...
With a high priority score of **${score}**, we recommend launching a **${scoringData.pilotAsk}** immediately. This will establish a secure sandboxed environment to validate compliance gates, trace data provenance natively, and secure final Quality Unit sign-off before full-scale commercialization.`;
  ...
  // --------------------------------------------------------------------------
  // JSX RENDERING ENGINE (100% CLEANED SLATE, NO DUPLICATE HEADERS)
  // --------------------------------------------------------------------------
  return (
    <div 
    ...
    >
      {/* Dynamic Sub-Header Info Bar (Thin, Non-Duplicated Metadata Indicator) */}
      <div
        id="v11-compact-metadata-ribbon"
        style={{
          display: 'flex',
          alignItems: 'center',
          justifyContent: 'space-between',
          padding: '0.45rem 1rem',
          background: 'rgba(18, 25, 44, 0.45)',
          borderRadius: '100px',
      ...
      >
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <span style={{ fontSize: '0.65rem', fontWeight: 900, background: 'rgba(56,189,248,0.15)', color: t.accent, padding: '0.2rem 0.6rem', borderRadius: '100px', letterSpacing: '0.5px' }}>
            ACTIVE DISCOVERY WORKLOAD
          </span>
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.45rem' }}>
            <span style={{ fontSize: '0.78rem', fontWeight: 800, color: t.textMain }}>
              {customerInfo.company || 'Enterprise Candidate'}
            </span>
            ...
            <span style={{ fontSize: '0.72rem', color: t.textMuted }}>
              {customerInfo.useCaseName || 'Drafting Scoping Profile'}
            </span>
          </div>
        </div>

        {/* Global Action Prefill / Switcher */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          {activeTab === 'intake' ? (
            ...
            >
              <Zap size={11} fill="#ffffff" /> Prefill GxP Sample
            </button>
          ) : (
            <button
      ...
      <main style={{ flex: 1, overflow: 'hidden', display: 'flex', flexDirection: 'column', position: 'relative' }}>
        ...
        {activeTab === 'intake' && (
          <div style={{ display: 'flex', gap: '1rem', flex: 1, overflow: 'hidden' }}>
            ...
            {/* Left Column: Glassmorphic Pillar Navigator (28% width) */}
            <div style={{ width: '28%', background: t.cardBg, border: t.cardBorder, borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem', backdropFilter: 'blur(20px)', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <Layers size={14} color={t.accent} />
                <h3 style={{ fontSize: '0.82rem', fontWeight: 900, color: t.textMain, textTransform: 'uppercase', letterSpacing: '0.5px', margin: 0 }}>
                  GxP Scoping Pillars
                </h3>
              </div>
                  ...
                  return (
                    <div 
                      ...
                      style={{
                        ...
                        borderRadius: '10px',
                        background: isActive ? 'rgba(56,189,248,0.07)' : 'rgba(255,255,255,0.01)',
                        border: isActive ? '1px solid rgba(56,189,248,0.25)' : '1px solid rgba(255,255,255,0.04)',
                        borderLeft: isActive ? `4px solid ${t.accent}` : '1px solid rgba(255,255,255,0.04)',
                        cursor: 'pointer',
                    ...
                    >
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.2rem' }}>
                        ...
                        {isCompleted && <CheckCircle2 size={12} color={t.accentGreen} />}
                      </div>
                      <div style={{ fontSize: '0.74rem', fontWeight: isActive ? 850 : 600, color: isActive ? t.textMain : t.textMuted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {p.name}
                      </div>
                    </div>
                  );
              ...
              {/* Overall Circular Progress Tracker */}
              <div style={{ borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '1rem', display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <div style={{ position: 'relative', width: '36px', height: '36px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <svg style={{ width: '100%', height: '100%', transform: 'rotate(-90deg)' }}>
                    ...
                    <circle cx="18" cy="18" r="15" fill="none" stroke={t.accent} strokeWidth="3" strokeDasharray={`${2 * Math.PI * 15}`} strokeDashoffset={`${2 * Math.PI * 15 * (1 - getOverallQuestionIndex() / 21)}`} style={{ transition: 'stroke-dashoffset 0.3s' }} />
                  </svg>
                  <span style={{ position: 'absolute', fontSize: '0.65rem', fontWeight: 900, color: t.textMain }}>
                    {Math.round((getOverallQuestionIndex() / 21) * 100)}%
                  </span>
                </div>
                <div>
                  <div style={{ fontSize: '0.75rem', fontWeight: 800, color: t.textMain }}>Scoping Progress</div>
                  <div style={{ fontSize: '0.65rem', color: t.textMuted }}>Question {getOverallQuestionIndex()} of 21</div>
                </div>
              </div>
            </div>

            {/* Right Column: Premium Glass Scoping Panel (72% width, Card-Locked height) */}
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
              <div style={{ background: t.cardBg, border: t.cardBorder, borderRadius: '16px', padding: '1.25rem', display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden', backdropFilter: 'blur(20px)', position: 'relative' }}>
                
                {/* Active Question Title & Scope */}
                <div style={{ marginBottom: '1.25rem', borderBottom: '1px solid rgba(255,255,255,0.06)', paddingBottom: '0.75rem' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.35rem' }}>
                    <span style={{ fontSize: '0.62rem', fontWeight: 950, color: t.accent, background: 'rgba(56,189,248,0.12)', padding: '0.15rem 0.55rem', borderRadius: '100px', border: '1px solid rgba(56,189,248,0.2)', textTransform: 'uppercase' }}>
                      {activePillar.name}
                    </span>
                    <span style={{ fontSize: '0.65rem', color: t.textMuted }}>
                      • Section {activePillar.id}
                    </span>
                  </div>
                  <h2 style={{ fontSize: '1rem', fontWeight: 850, margin: 0, color: t.textMain, lineHeight: 1.4 }}>
                    {activeQuestion.topic}
                  </h2>
                </div>

                {/* Split Interactive Scoping Panel */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1, overflowY: 'auto' }}>
                  
                  {/* Top Half: Modern Rating Dials */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', background: 'rgba(0,0,0,0.12)', padding: '1rem', borderRadius: '12px', border: '1px solid rgba(255,255,255,0.04)' }}>
                    
                    {/* Current State Dial Row */}
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.5rem' }}>
                        <span style={{ fontSize: '0.7rem', fontWeight: 900, color: t.accent, letterSpacing: '0.5px' }}>CURRENT COMPLIANCE LEVEL</span>
                        <span style={{ fontSize: '0.6rem', background: 'rgba(56,189,248,0.1)', color: t.accent, padding: '0.05rem 0.3rem', borderRadius: '4px' }}>As-Is</span>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                height: '36px',
                                borderRadius: '8px',
                                background: isActive ? t.pillCurrent : 'rgba(255,255,255,0.02)',
                                ...
                                boxShadow: isActive ? '0 0 10px rgba(14,165,233,0.4)' : 'none'
                              }}
                            >
                              {idx}
                            </button>
                      ...
                      <div style={{ minHeight: '40px', background: 'rgba(0,0,0,0.2)', padding: '0.4rem 0.6rem', borderRadius: '6px', border: '1px solid rgba(255,255,255,0.03)', fontSize: '0.68rem', color: t.textMuted, lineHeight: 1.4 }}>
                        {activeQuestion.options[hoveredOptionIdx !== null ? hoveredOptionIdx : ((scores[activeQuestionId]?.current || 3) - 1)]?.text}
                      </div>
                    </div>

                    {/* Target State Dial Row */}
                    <div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: '0.35rem', marginBottom: '0.5rem' }}>
                        <span style={{ fontSize: '0.7rem', fontWeight: 900, color: t.accentGreen, letterSpacing: '0.5px' }}>TARGET SCOPING GOAL</span>
                        <span style={{ fontSize: '0.6rem', background: 'rgba(16,185,129,0.1)', color: t.accentGreen, padding: '0.05rem 0.3rem', borderRadius: '4px' }}>To-Be</span>
                      </div>
                          ...
                          return (
                            <button
                              ...
                              style={{
                                ...
                                height: '36px',
                                borderRadius: '8px',
                                background: isActive ? t.pillTarget : 'rgba(255,255,255,0.02)',
                                border: isActive ? 'none' : '1px solid rgba(255,255,255,0.06)',
                                color: isActive ? '#ffffff' : t.textMain,
                                fontSize: '0.85rem',
                                fontWeight: 900,
                  ...
                  {/* Bottom Half: Interactive Clickable Painpoint Tags & Notes */}
                  <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 1fr', gap: '1.5rem', flex: 1, minHeight: '180px' }}>
                    
                    {/* Pain Points tags grid */}
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                      <span style={{ fontSize: '0.7rem', fontWeight: 900, color: t.textMuted, letterSpacing: '0.5px' }}>
                        ACTIVE DISCOVERY PAIN POINTS (TAP TO TOGGLE)
                      </span>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '0.35rem', content: 'flex-start' }}>
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                border: isChecked ? '1px solid #f59e0b' : '1px solid rgba(255,255,255,0.06)',
                                color: isChecked ? '#f59e0b' : t.textMuted,
                                fontSize: '0.68rem',
                                fontWeight: isChecked ? 800 : 550,
                                cursor: 'pointer',
                                transition: 'all 0.2s',
                                display: 'flex',
                                alignItems: 'center',
                          ...
                          return (
                            <div
                              ...
                              style={{
                                ...
                                border: isChecked ? '1px solid #ef4444' : '1px solid rgba(255,255,255,0.06)',
                                color: isChecked ? '#ef4444' : t.textMuted,
                                fontSize: '0.68rem',
                                fontWeight: isChecked ? 800 : 550,
                                cursor: 'pointer',
                                transition: 'all 0.2s',
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '0.85rem', marginTop: '0.85rem', flexShrink: 0 }}>
                  <button
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(clean_premium_code)

print("PremiumScopingAssessorV11 has been successfully redesigned from scratch as an ultra-premium, clean boardroom masterpiece!")
