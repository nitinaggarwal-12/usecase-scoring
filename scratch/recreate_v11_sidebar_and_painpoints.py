...
aligned_code = """import { useState, useMemo, useEffect, useRef, useCallback } from 'react';
import { 
...
With a high priority score of **${score}**, we recommend launching a **${scoringData.pilotAsk}** immediately. This will establish a secure sandboxed environment to validate compliance gates, trace data provenance natively, and secure final Quality Unit sign-off before full-scale commercialization.`;
  ...
  // --------------------------------------------------------------------------
  // JSX RENDERING ENGINE (100% LIGHT-THEMED 4-COLUMN AUDIT BOARD)
  // --------------------------------------------------------------------------
  return (
    <div 
    ...
    >
      {/* LEFT SIDEBAR PANEL (Width: 260px, White background, crisp branding) */}
      <aside
        style={{
          width: '260px',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
          {/* Header Identity */}
          <div>
            <h2 style={{ fontSize: '0.85rem', fontWeight: 900, color: colors.textDark, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 0.15rem 0' }}>
            ...
            <div style={{ height: '4px', background: '#e2e8f0', borderRadius: '100px', marginTop: '0.5rem', overflow: 'hidden' }}>
              <div style={{ width: `${Math.round((getOverallQuestionIndex() / 21) * 100)}%`, height: '100%', background: colors.accentBlue }}></div>
            </div>
          </div>

          {/* Pillars List (Corrected to exact V11 GxP Pillar names!) */}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', overflow: 'hidden' }}>
                    <span style={{ fontSize: '0.8rem', color: isActive ? colors.accentBlue : colors.textMuted }}>
                      {idx === 0 ? '📊' : idx === 1 ? '🧠' : idx === 2 ? '🔌' : idx === 3 ? '⚙️' : '🛡️'}
                    </span>
                    <span style={{ fontSize: '0.74rem', fontWeight: isActive ? 800 : 550, color: isActive ? colors.textDark : colors.textMuted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {pillar.name}
                    </span>
                  </div>
                  <span style={{ fontSize: '0.65rem', fontWeight: 800, color: colors.textMuted }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem', borderTop: `1px solid ${colors.borderLight}`, paddingTop: '1rem' }}>
          
          {/* Offline Sync Controls */}
          <div>
            <span style={{ fontSize: '0.6rem', fontWeight: 800, color: colors.textMuted, textTransform: 'uppercase', display: 'block', marginBottom: '0.35rem' }}>
              OFFLINE SHEET SYNC
            </span>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.4rem' }}>
              <button style={{ background: '#ffffff', border: `1px solid ${colors.borderLight}`, color: colors.accentBlue, borderRadius: '6px', padding: '0.35rem 0', fontSize: '0.68rem', fontWeight: 800, cursor: 'pointer' }}>
                📥 Export
              </button>
              <button style={{ background: '#ffffff', border: `1px solid ${colors.borderLight}`, color: '#16a34a', borderRadius: '6px', padding: '0.35rem 0', fontSize: '0.68rem', fontWeight: 800, cursor: 'pointer' }}>
                📤 Import
              </button>
            </div>
          </div>
      ...
      <main style={{ flex: 1, padding: '1.25rem 1.5rem', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {activeTab === 'intake' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem', flex: 1 }}>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: `1px solid ${colors.borderLight}`, paddingBottom: '0.75rem' }}>
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginBottom: '0.2rem' }}>
                  <span style={{ fontSize: '0.8rem', fontWeight: 900, color: colors.textDark }}>
                    📊 {activePillar.name}
                  </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                ...
                <span style={{ fontSize: '0.62rem', background: '#dcfce7', color: '#15803d', padding: '0.15rem 0.5rem', borderRadius: '4px', fontWeight: 900 }}>
                  • SAVED
                </span>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.72rem', color: colors.textMuted, cursor: 'pointer' }}>
                  <input type="checkbox" /> Skip Topic
                </label>
              </div>
            </div>
            ...
            {/* Active Topic Header */}
            <div>
              <span style={{ fontSize: '0.68rem', fontWeight: 900, color: colors.accentBlue, textTransform: 'uppercase', display: 'block', letterSpacing: '0.5px', marginBottom: '0.2rem' }}>
                TOPIC INDEX STRATEGIC.{getOverallQuestionIndex()} ASSESSMENT SCRUTINY:
              </span>
              <h2 style={{ fontSize: '1rem', fontWeight: 900, margin: 0, color: colors.textDark }}>
                {activeQuestion.topic}
              </h2>
            </div>

            {/* 4-COLUMN COMPLIANCE AUDIT MATRIX GRID */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '1rem', flex: 1 }}>
              
              {/* COLUMN 1: CURRENT STATE */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 850, color: colors.textDark, borderBottom: `2px solid ${colors.textDark}`, paddingBottom: '0.25rem', display: 'block' }}>
                  CURRENT STATE *
                </span>
              ...
              {/* COLUMN 3: TECHNICAL PAINPOINTS (Enriched to at least 5 options!) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 850, color: colors.textDark, borderBottom: `2px solid ${colors.textDark}`, paddingBottom: '0.25rem', display: 'block' }}>
              ...
              {/* COLUMN 4: BUSINESS PAINPOINTS (Enriched to at least 5 options!) */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                <span style={{ fontSize: '0.72rem', fontWeight: 850, color: colors.textDark, borderBottom: `2px solid ${colors.textDark}`, paddingBottom: '0.25rem', display: 'block' }}>
                  BUSINESS PAINPOINTS *
                </span>
            ...
            <div style={{ background: colors.cardBg, border: `1px solid ${colors.borderLight}`, borderRadius: '12px', padding: '1rem', display: 'flex', flexDirection: 'column', gap: '0.35rem' }}>
              <span style={{ fontSize: '0.72rem', fontWeight: 900, color: colors.textDark, letterSpacing: '0.5px' }}>
                NOTES & COMMENTS BOX *
              </span>
              <textarea
            ...
            {/* Scrutiny Wizard Navigation Footer */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${colors.borderLight}`, paddingTop: '0.75rem', flexShrink: 0 }}>
              <button
              ...
              <span style={{ fontSize: '0.7rem', color: colors.textMuted, fontWeight: 700 }}>
...
print("PremiumScopingAssessorV11 has been successfully aligned with the correct V11 pillars and enriched with >= 5 pain points per question!")
