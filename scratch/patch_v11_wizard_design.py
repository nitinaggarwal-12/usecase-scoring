...
# 1. Inject state hooks and handlers
# Let's find where answers state is declared
state_declaration_marker = "  const [scores, setScores] = useState({});"
state_declaration_idx = content.find(state_declaration_marker)

new_states_and_handlers = """  const [scores, setScores] = useState({});
  const [activeQuestionIdx, setActiveQuestionIdx] = useState(0);

  const handleNext = () => {
    const isBusiness = activeTab === 'business';
    const pillars = V11_PILLARS.filter(p => isBusiness ? p.persona === 'Business' : p.persona !== 'Business');
    const activePillar = pillars.find(p => p.id === activeDimensionId) || pillars[0];
    const pillarIdx = pillars.findIndex(p => p.id === activePillar.id);
    
    if (activeQuestionIdx < activePillar.questions.length - 1) {
      setActiveQuestionIdx(activeQuestionIdx + 1);
    } else {
      // Last question of this pillar
      if (pillarIdx < pillars.length - 1) {
        // Move to next pillar in this tab
        const nextPillar = pillars[pillarIdx + 1];
        setActiveDimensionId(nextPillar.id);
        setActiveQuestionIdx(0);
      } else {
        // Last question of the last pillar of this tab
        if (isBusiness) {
          // Switch to Technical Tab
          setActiveTab('technical');
          const techPillars = V11_PILLARS.filter(p => p.persona !== 'Business');
          setActiveDimensionId(techPillars[0].id);
          setActiveQuestionIdx(0);
        } else {
          // Technical Tab - Last Question! Trigger compilation!
  ...
  const handleBack = () => {
    ...
    if (activeQuestionIdx > 0) {
      ...
      // First question of this pillar
      if (pillarIdx > 0) {
        // Move to previous pillar in this tab
        const prevPillar = pillars[pillarIdx - 1];
        setActiveDimensionId(prevPillar.id);
        setActiveQuestionIdx(prevPillar.questions.length - 1);
      } else {
        // First question of the first pillar of this tab
        if (!isBusiness) {
          // Go back to Business Tab
          setActiveTab('business');
          const bizPillars = V11_PILLARS.filter(p => p.persona === 'Business');
          const lastBizPillar = bizPillars[bizPillars.length - 1];
          setActiveDimensionId(lastBizPillar.id);
          setActiveQuestionIdx(lastBizPillar.questions.length - 1);
        } else {
          // Go back to Metadata gate
          setActiveTab('intake');
        }
      }
    }
...
if state_declaration_idx != -1:
    content = content[:state_declaration_idx] + new_states_and_handlers + content[state_declaration_idx + len(state_declaration_marker):]

# 2. Reset activeQuestionIdx to 0 when clicking top tabs/pillars
# Find the pill click handlers for Business persona
# They look like: onClick={() => setActiveDimensionId(p.id)}
# Let's replace them to also reset activeQuestionIdx to 0
content = content.replace("onClick={() => setActiveDimensionId(p.id)}", "onClick={() => { setActiveDimensionId(p.id); setActiveQuestionIdx(0); }}")

# 3. Swap the questionnaire rendering blocks with the single-question wizard card!
...
# Find Business Loop
biz_marker = "{/* Individual Question Card */}"
start_idx = content.find(biz_marker)
if start_idx != -1:
    end_idx = content.find("</div>", content.find("activePillar.questions.map", start_idx))
    # We want to replace everything from the biz_marker up to the end of the div (which terminates the loop)
    # Since we previously patched it to index-based, we can easily locate the end
    end_marker = "                })}\n              </div>"
...
if start_idx_tech != -1:
    end_marker = "                })}\n              </div>"
    end_idx_tech = content.find(end_marker, start_idx_tech)
    if end_idx_tech != -1:
        end_idx_tech += len(end_marker)
        content = content[:start_idx_tech] + new_wizard_loop_code.replace("Individual Question Card", "Individual Technical Question Card") + content[end_idx_tech:]
        print("Technical loop successfully patched to wizard!")

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Wizard successfully integrated into V11 Assessor!")
