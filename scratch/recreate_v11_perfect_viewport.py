...
perfect_viewport_code = """import { useState, useMemo, useEffect, useRef, useCallback } from 'react';
import { 
...
With a high priority score of **${score}**, we recommend launching a **${scoringData.pilotAsk}** immediately. This will establish a secure sandboxed environment to validate compliance gates, trace data provenance natively, and secure final Quality Unit sign-off before full-scale commercialization.`;
  ...
  // --------------------------------------------------------------------------
  // JSX RENDERING ENGINE (100% LIGHT-THEMED VIEWPORT-LOCKED AUDIT BOARD)
  // --------------------------------------------------------------------------
  return (
    <div 
      ...
      style={{
        ...
        height: 'calc(100vh - 75px)', // Lock height to viewport to prevent outer scrollbar
        maxHeight: 'calc(100vh - 75px)',
        display: 'flex',
        overflow: 'hidden', // Eradicate page-level scrollbars
        position: 'relative'
      }}
    >
      {/* LEFT SIDEBAR PANEL (Viewport-locked, compact packed spacing) */}
      <aside
        style={{
          width: '240px',
      ...
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
          {/* Header Identity */}
          <div>
            <h2 style={{ fontSize: '0.78rem', fontWeight: 900, color: colors.textDark, textTransform: 'uppercase', letterSpacing: '0.5px', margin: '0 0 0.15rem 0' }}>
            ...
            <div style={{ height: '4px', background: '#e2e8f0', borderRadius: '100px', marginTop: '0.35rem', overflow: 'hidden' }}>
              <div style={{ width: `${Math.round((getOverallQuestionIndex() / 21) * 100)}%`, height: '100%', background: colors.accentBlue }}></div>
            </div>
          </div>

          {/* Pillars List (Compact padding to eliminate vertical spacing gaps) */}
              ...
              return (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', overflow: 'hidden' }}>
                    <span style={{ fontSize: '0.75rem', color: isActive ? colors.accentBlue : colors.textMuted }}>
                      {idx === 0 ? '📊' : idx === 1 ? '🧠' : idx === 2 ? '🔌' : idx === 3 ? '⚙️' : '🛡️'}
                    </span>
                    <span style={{ fontSize: '0.7rem', fontWeight: isActive ? 800 : 550, color: isActive ? colors.textDark : colors.textMuted, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {pillar.name}
                    </span>
                  </div>
                  <span style={{ fontSize: '0.62rem', fontWeight: 800, color: colors.textMuted }}>
        ...
        {/* Sidebar Footer Operations (Packed neatly, no massive blank center gap) */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.5rem', borderTop: `1px solid ${colors.borderLight}`, paddingTop: '0.75rem' }}>
          
          {/* Offline Sync Controls */}
          <div>
            <span style={{ fontSize: '0.55rem', fontWeight: 800, color: colors.textMuted, textTransform: 'uppercase', display: 'block', marginBottom: '0.25rem' }}>
              OFFLINE SHEET SYNC
            </span>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0.3rem' }}>
              <button style={{ background: '#ffffff', border: `1px solid ${colors.borderLight}`, color: colors.accentBlue, borderRadius: '4px', padding: '0.25rem 0', fontSize: '0.65rem', fontWeight: 800, cursor: 'pointer' }}>
                📥 Export
              </button>
              <button style={{ background: '#ffffff', border: `1px solid ${colors.borderLight}`, color: '#16a34a', borderRadius: '4px', padding: '0.25rem 0', fontSize: '0.65rem', fontWeight: 800, cursor: 'pointer' }}>
                📤 Import
              </button>
            </div>
          </div>
      ...
      {/* RIGHT WORKSPACE AREA (Viewport-locked flex layout) */}
      <main style={{ flex: 1, padding: '0.75rem 1rem', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
        
        {/* VIEW 1: INTAKE 4-COLUMN SCRUTINY WORKSPACE */}
        {activeTab === 'intake' && (
          <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', justifyBetween: 'space-between' }}>
            
            {/* Topic Pillar Title bar */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderBottom: `1px solid ${colors.borderLight}`, paddingBottom: '0.4rem', marginBottom: '0.5rem', flexShrink: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                <span style={{ fontSize: '0.75rem', fontWeight: 900, color: colors.textDark }}>
                  📊 {activePillar.name}
                </span>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                ...
                <span style={{ fontSize: '0.55rem', background: '#dcfce7', color: '#15803d', padding: '0.1rem 0.4rem', borderRadius: '3px', fontWeight: 900 }}>
                  • SAVED
                </span>
                <label style={{ display: 'flex', alignItems: 'center', gap: '0.2rem', fontSize: '0.65rem', color: colors.textMuted, cursor: 'pointer' }}>
            ...
            <div style={{ marginBottom: '0.5rem', flexShrink: 0 }}>
              <span style={{ fontSize: '0.62rem', fontWeight: 900, color: colors.accentBlue, textTransform: 'uppercase', display: 'block', letterSpacing: '0.5px', marginBottom: '0.1rem' }}>
                TOPIC INDEX STRATEGIC.{getOverallQuestionIndex()} ASSESSMENT SCRUTINY:
              </span>
              <h2 style={{ fontSize: '0.85rem', fontWeight: 900, margin: 0, color: colors.textDark, lineHeight: 1.3 }}>
                {activeQuestion.topic}
              </h2>
            </div>

            {/* 4-COLUMN COMPLIANCE AUDIT MATRIX GRID (Packed natural height, no vertical stretch) */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.6rem', flexShrink: 0, marginBottom: '0.6rem' }}>
              
              {/* COLUMN 1: CURRENT STATE */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                <span style={{ fontSize: '0.65rem', fontWeight: 850, color: colors.textDark, borderBottom: `2px solid ${colors.textDark}`, paddingBottom: '0.15rem', display: 'block' }}>
                  CURRENT STATE *
                </span>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.3rem' }}>
                  {activeQuestion.technicalPainpoints?.map(point => {
                    ...
                    return (
                      ...
                      >
                        <input type="checkbox" checked={isChecked} readOnly style={{ cursor: 'pointer', margin: 0 }} />
                        <span>{point}</span>
                      </div>
                    );
                  })}
            ...
            {/* Bottom Auditor Notes Area (Slides right up below the grid, no empty gaps!) */}
            <div style={{ background: colors.cardBg, border: `1px solid ${colors.borderLight}`, borderRadius: '8px', padding: '0.5rem 0.75rem', display: 'flex', flexDirection: 'column', gap: '0.2rem', flexShrink: 0, marginBottom: '0.5rem' }}>
              <span style={{ fontSize: '0.65rem', fontWeight: 900, color: colors.textDark, letterSpacing: '0.5px' }}>
                NOTES & COMMENTS BOX *
              </span>
              <textarea
                ...
                style={{
                  width: '100%',
                  height: '45px', // Extremely compact fixed height
                  border: 'none',
                  outline: 'none',
                  fontSize: '0.7rem',
                  color: colors.textDark,
            ...
            {/* Scrutiny Wizard Navigation Footer (Pinned cleanly at the viewport bottom) */}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: `1px solid ${colors.borderLight}`, paddingTop: '0.4rem', flexShrink: 0 }}>
              <button
              ...
              <button
                ...
                disabled={getOverallQuestionIndex() === 21}
                style={{
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(perfect_viewport_code)

print("PremiumScopingAssessorV11 has been successfully locked to the viewport and completely optimized to eliminate empty space and scrolling!")
