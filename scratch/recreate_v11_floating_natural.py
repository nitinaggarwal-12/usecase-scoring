...
floating_natural_code = """import { useState, useMemo, useEffect, useRef, useCallback } from 'react';
import { 
...
With a high priority score of **${score}**, we recommend launching a **${scoringData.pilotAsk}** immediately. This will establish a secure sandboxed environment to validate compliance gates, trace data provenance natively, and secure final Quality Unit sign-off before full-scale commercialization.`;
  ...
  // --------------------------------------------------------------------------
  // JSX RENDERING ENGINE (100% LIGHT-THEMED FLOATING NATURAL CARD WORKSPACE)
  // --------------------------------------------------------------------------
  return (
    <div 
      ...
      style={{
        ...
        display: 'flex',
        alignItems: 'flex-start', // Let elements flow naturally without vertical stretching
        gap: '0.85rem', // Crisp, clean panel gap
        padding: '1rem', // Natural breathing margin around the cards
        boxSizing: 'border-box',
        minHeight: '100vh', // Ensure background fills page
        overflowY: 'auto' // Natural page scrolling if screen is extremely tiny
      }}
    >
      {/* LEFT SIDEBAR PANEL (Natural floating white card, wrapping content tightly!) */}
      <aside
        style={{
          width: '235px',
          background: colors.sidebarBg,
          border: `1px solid ${colors.borderLight}`,
          borderRadius: '12px', // High-fidelity card corners
          padding: '0.85rem',
          display: 'flex',
          flexDirection: 'column',
          gap: '0.85rem', // Tightly stack elements naturally from top
          flexShrink: 0,
          boxShadow: '0 4px 12px rgba(15, 23, 42, 0.03)', // Premium soft floating shadow
          boxSizing: 'border-box',
          height: 'fit-content' // CARD ENDS NATURALLY right below contents! No giant empty space!
        }}
      >
        {/* Header Identity */}
        <div>
        ...
        {/* Divider line */}
        <div style={{ borderTop: `1px solid ${colors.borderLight}` }}></div>
      ...
      {/* RIGHT MAIN WORKSPACE (Natural floating white card, wrapping content tightly!) */}
      <main style={{ flex: 1, display: 'flex', flexDirection: 'column', boxSizing: 'border-box' }}>
        ...
        {activeTab === 'intake' && (
          <div 
            style={{ 
              ...
              display: 'flex', 
              flexDirection: 'column', 
              gap: '0.55rem', // Tight vertical stacking
              boxShadow: '0 4px 12px rgba(15, 23, 42, 0.03)', // Premium soft shadow
              boxSizing: 'border-box',
              height: 'fit-content' // CARD ENDS NATURALLY right below the navigation footer! NO giant empty space!
            }}
          >
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '0.5rem', flexShrink: 0, marginBottom: '0.25rem' }}>
            ...
            {/* Navigation Footer (Sits NATURALLY right below the notes box!) */}
...
with open(file_path, 'w', encoding='utf-8') as f:
    f.write(floating_natural_code)

print("PremiumScopingAssessorV11 has been successfully recreated to match the authentic V5/V9 floating natural card layout!")
